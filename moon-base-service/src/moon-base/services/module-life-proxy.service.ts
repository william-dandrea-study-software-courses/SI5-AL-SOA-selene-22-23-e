...
export class ModuleLifeProxyService {
  ...
  async getNeeds(): Promise<NeedsDto> {
    try {
      const retrieveModulesNeedsCallResponse: AxiosResponse<NeedsDto> =
        await firstValueFrom(
          this.httpService.get(`${this._baseUrl}${this._moduleLifePath}/needs`)
        );
  ...
  async getInventory(): Promise<InventoryDto> {
    try {
      const retrieveModulesInventoryCallResponse: AxiosResponse<InventoryDto> =
        await firstValueFrom(
          this.httpService.get(
            `${this._baseUrl}${this._moduleLifePath}/inventory`
          )
        );
  ...
  async isolate(moduleId: number) {
    try {
      const retrieveIsolateModuleCallResponse: AxiosResponse<any> =
        await firstValueFrom(
          this.httpService.put(
            `${this._baseUrl}${this._moduleLifePath}/${moduleId}/isolate`
          )
        );
    } catch (exception) {
      throw exception;
