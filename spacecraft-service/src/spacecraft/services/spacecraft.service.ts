...
import { HttpService, AxiosError } from "@nestjs/axios";
...
export class SpacecraftService {
  ...
  constructor(
      @InjectModel(SpaceCraft.name) private spaceCraftModel: Model<SpaceCraftDocument>,
  ...
  async postSpaceCraft(spacecraftDTO: SpacecraftDto): Promise<SpacecraftDto> {
    ...
    if(spacecraftDTO.vitals == false || spacecraftDTO.status === StatusSpacecraftEnumSchema.DAMAGED){
      ...
      await producer.send({
        ...
        messages: [
          { key: 'resupply_mission_id',value:spacecraftDTO.id_resupplyMission},
        ],
      });
  ...
  async affectSpaceCraftToMission(spacecraft_id:string, resupplyMissionId: string): Promise<SpacecraftDto> {
    const spaceCrafts = await this.spaceCraftModel.findOne({spacecraft_id: spacecraft_id,status: StatusSpacecraftEnumSchema.PREPARING, id_resupplyMission: "None"})
    ...
    try {
      this.logger.log("Send resupply mission at adress : "+this._baseUrlResupply + "/resupply/"+resupplyMissionId+"/affectSpacecraft")
      const retrieveModuleStatusResponse: AxiosResponse<any> =
          await firstValueFrom(
              this.httpService.put(this._baseUrlResupply + "/resupply/" + resupplyMissionId + "/affectSpacecraft", {spacecraft_id: spacecraft_id})
          );
    }
    catch(exception){
  ...
  async launchSpaceCraft(idSpaceCraft: number): Promise<SpacecraftDto> {
    ...
    if(spaceCrafts.status !== StatusSpacecraftEnumSchema.PREPARING){
      throw new HttpException("Vous ne pouvez pas lancer un spacecraft qui n'est pas en préparation", HttpStatus.FORBIDDEN );
    }

    spaceCrafts.status = StatusSpacecraftEnumSchema.TRAVELING;
    await spaceCrafts.save();
    this.logger.log("Send resupply mission at adress : "+this._baseUrlResupply + "/resupply/"+spaceCrafts.id_resupplyMission+"/send")
